#!/bin/bash
sed -i 's/\r//g' ppt.sh
if [ ! -e "/etc/init.d/chatGptWarpRestall" ]; then
### BEGIN INIT INFO
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
### END INIT INFO
wget -N --no-check-certificate https://gitlab.com/rwkgyg/CFwarp/raw/main/CFwarp.sh
cp warpRestall.sh /etc/init.d/chatGptWarpRestall
sudo chmod +x /etc/init.d/chatGptWarpRestall
sudo update-rc.d chatGptWarpRestall defaults
fi
echo $(date "+%Y-%m-%d %H:%M:%S")+"chatGptWarpRestall startting">>/root/chatGptService.log
