#!/bin/bash
sed -i 's/\r//g' ppt.sh
if [ ! -e "/etc/init.d/chatGptWarpRestart" ]; then
### BEGIN INIT INFO
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
### END INIT INFO
wget -N --no-check-certificate https://gitlab.com/rwkgyg/CFwarp/raw/main/CFwarp.sh
cp warpRestart.sh /etc/init.d/chatGptWarpRestart
sudo chmod +x /etc/init.d/chatGptWarpRestart
sudo update-rc.d chatGptWarpRestart defaults
fi
echo $(date "+%Y-%m-%d %H:%M:%S")+"RestartScript startting">>/root/chatGptService.log
